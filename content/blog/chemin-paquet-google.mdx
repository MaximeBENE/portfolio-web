...
Dans mon lab, les deux VMs sont sur le même réseau `192.168.244.0/24`. Elles devraient se voir en ARP.
...
- Est-elle dans la **bonne plage** ? (dans mon lab : `192.168.244.x`)
...
Dans mon lab, la passerelle est `192.168.244.2` (VirtualBox).
...
**Ce que je regarde :**
- Y a-t-il une ligne `default via 192.168.244.2` ?
...
```bash
sudo ip route add default via 192.168.244.2
```
...
- [Le modèle OSI : comment j'ai arrêté de redémarrer tout et n'importe quoi](/blog/modele-osi-depannage)
